...
# ============================================================
# 💬 VaxShield AI Chatbot (Final Local Model Version – Stable)
# ============================================================
...
def handle_user_message(user_input, reanalyze=False):
    ...
    if intent == "greeting":
        reply = random.choice([
            "👋 Hi there! I'm VaxShield AI — here to validate vaccine reaction models.",
            "Hello! You can describe a vaccine reaction, and I'll predict its seriousness.",
            "Hey! Try: 'A 45-year-old male took the flu shot and felt dizzy.'"
        ])

    elif intent == "info":
        reply = (
            "💡 Common mild reactions: fatigue, soreness, fever, headache.\n"
            "Serious ones (hospitalization, paralysis, death) are rare — our CatBoost models classify their seriousness."
        )

    elif intent == "prediction":
        with st.spinner("🔍 Analyzing using trained CatBoost model..."):
            time.sleep(1.2)
    ...
    # ✅ use stable rerun (fixes deprecated experimental)
    st.rerun()
...
def summarize_prediction(prediction_text):
    ...
    if "Severe" in prediction_text or "High Seriousness" in prediction_text:
        tone = random.choice([
            ...
            "❗ Model flagged this as severe — human evaluation recommended."
        ])
