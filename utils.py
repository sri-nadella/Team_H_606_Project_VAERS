...
# ============================================================
# 🔹 Entity Extraction (robust + fixed)
# ============================================================
def extract_entities(text):
    ...
    # --- Corrected cross-inference ---
    # Apply only when the age isn't explicitly mentioned or is very low (<40)
    if age_flag != "exact":
        if sex == "F" and re.search(r"\bold\b|\belderly\b|\bgrandmother\b", text_lower):
            age = 70
        if sex == "M" and re.search(r"\bold\b|\belderly\b|\bgrandfather\b", text_lower):
            age = 70
...
# ============================================================
# 🔹 Prediction Logic (clean + debug-safe)
# ============================================================
def predict_seriousness(text, interactive=False):
    vaccine, age, sex, symptoms, age_flag = extract_entities(text)
    debug_mode = st.session_state.get("show_debug", True)

    # --- Interactive age prompt ---
    ...
    if not vaccine:
        st.error("⚠️ Could not identify vaccine type.")
    ...
    try:
        ...
        text_feats = [f for f in model.feature_names_
    ...
    if debug_mode:
        st.info(f"📊 **Model Confidence:** {pct}% serious probability from `{os.path.basename(model_path)}`", icon="🤖")
...
# ============================================================
# 🔹 Theme Utility (modern, fixed)
# ============================================================
def apply_theme(theme="dark", accent="#0055aa", font="Inter", compact=False):
    """Enhanced reactive theme injection with accent + font + compact mode."""
